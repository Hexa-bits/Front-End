...
// ../node_modules/fetch-mock/dist/esm/index.js
var FetchMock$3 = {};
...
FetchMock$3.addRoute = function(uncompiledRoute) {
  ...
  if (this.getOption("overwriteRoutes", route) === true) {
    clashes.forEach((clash) => {
      const index2 = this.routes.indexOf(clash);
      this._uncompiledRoutes.splice(index2, 1, uncompiledRoute);
      this.routes.splice(index2, 1, route);
    });
    return this.routes;
  }
  if (clashes.length) {
...
var defineShorthand = (methodName, underlyingMethod, shorthandOptions) => {
...
var defineGreedyShorthand = (methodName, underlyingMethod) => {
...
var getRouteRemover = ({ sticky: removeStickyRoutes }) => (routes) => removeStickyRoutes ? [] : routes.filter(({ sticky }) => sticky);
...
var ResponseBuilder = class {
  ...
  sendAsObject() {
    if (responseConfigProps.some((prop) => this.responseConfig[prop])) {
      if (Object.keys(this.responseConfig).every(
        (key) => responseConfigProps.includes(key)
      )) {
        return false;
      }
      return true;
...
var headersToArray = (headers2) => {
  if (typeof headers2.raw === "function") {
    return Object.entries(headers2.raw());
  }
...
var zipObject = (entries) => entries.reduce((obj, [key, val]) => Object.assign(obj, { [key]: val }), {});
function normalizeUrl(url) {
  ...
  if (protocolRelativeUrlRX.test(url)) {
    const u2 = new URL(url, "http://dummy");
    return u2.href;
  }
...
function normalizeRequest(url, options, Request) {
  if (Request.prototype.isPrototypeOf(url)) {
    ...
    const headers2 = headersToArray(url.headers);
    if (headers2.length) {
      normalizedRequestObject.options.headers = zipObject(headers2);
    }
    return normalizedRequestObject;
  }
  if (typeof url === "string" || url instanceof String || // horrible URL object duck-typing
...
var headers = {
  normalize: (headers2) => zipObject(headersToArray(headers2)),
  toLowerCase: (headers2) => Object.keys(headers2).reduce((obj, k) => {
    obj[k.toLowerCase()] = headers2[k];
    return obj;
  }, {}),
...
var FetchMock$2 = {};
var resolve = async ({ response, responseIsFetch = false }, url, options, request) => {
  while (true) {
...
FetchMock$2.executeRouter = function(url, options, request) {
  ...
  if (!this.getOption("fallbackToNetwork")) {
    throw new Error(
      `fetch-mock: No fallback response defined for ${options && options.method || "GET"} to ${url}`
    );
  }
  return {
    route: { response: this.getNativeFetch(), responseIsFetch: true },
...
FetchMock$2.router = function(url, options, request) {
  const route = this.routes.find((route2) => {
    return route2.matcher(url, options, request);
  });
  if (route) {
    return route;
  }
...
var globToRegexp = function(glob2, opts) {
  if (typeof glob2 !== "string") {
    throw new TypeError("Expected a string");
  }
  var str = String(glob2);
  var reStr = "";
...
(function(module, exports) {
  ...
  var isSubset2 = function(_isSubset) {
    function isSubset3(_x, _x2) {
      return _isSubset.apply(this, arguments);
    }
    isSubset3.toString = function() {
    ...
    return Object.keys(subset).every(function(key) {
      ...
      if (typeof subsetItem === "object" && subsetItem !== null ? !isSubset2(supersetItem, subsetItem) : supersetItem !== subsetItem) return false;
      return true;
    });
  });
  exports["default"] = isSubset2;
...
var debuggableUrlFunc = (func) => (url) => {
  return func(url);
};
var stringMatchers = {
...
var getHeaderMatcher = ({ headers: expectedHeaders }) => {
  if (!expectedHeaders) {
    return;
  }
  ...
  return (url, { headers: headers$1 = {} }) => {
    ...
    return Object.keys(expectation).every(
      (headerName) => headers.equal(lowerCaseHeaders[headerName], expectation[headerName])
    );
  };
};
var getMethodMatcher = ({ method: expectedMethod }) => {
  if (!expectedMethod) {
    return;
  }
  return (url, { method }) => {
...
var getQueryStringMatcher = ({ query: passedQuery }) => {
  if (!passedQuery) {
    return;
  }
    ...
    if (Array.isArray(value)) {
      for (const item of value) {
        expectedQuery.append(
          key,
          typeof item === "object" || typeof item === "undefined" ? "" : item.toString()
        );
      }
    } else {
      expectedQuery.append(
        key,
        typeof value === "object" || typeof value === "undefined" ? "" : value.toString()
      );
    }
  }
    ...
    return keys.every((key) => {
      ...
      if (Array.isArray(passedQuery[key])) {
        return expectedValues.every(
          (expected, index2) => expected === actualValues[index2]
        );
      }
...
var getParamsMatcher = ({ params: expectedParams, url: matcherUrl }) => {
  if (!expectedParams) {
    return;
  }
  ...
  return (url) => {
    ...
    const params = re.keys.reduce(
      (map, paramName, i) => vals[i] ? Object.assign(map, { [paramName]: vals[i] }) : map,
      {}
    );
    return expectedKeys.every((key) => params[key] === expectedParams[key]);
  };
};
var getBodyMatcher = (route, fetchMock) => {
...
var getFullUrlMatcher = (route, matcherUrl, query) => {
  ...
  return (matcherUrl2) => {
    if (query && expectedUrl.indexOf("?")) {
      return matcherUrl2.indexOf(expectedUrl) === 0;
    }
    return normalizeUrl(matcherUrl2) === expectedUrl;
  };
};
var getFunctionMatcher = ({ functionMatcher }) => {
  return (...args) => {
    return functionMatcher(...args);
  };
};
var getUrlMatcher = (route) => {
...
var isUrlMatcher = (matcher) => matcher instanceof RegExp || typeof matcher === "string" || typeof matcher === "object" && "href" in matcher;
var isFunctionMatcher = (matcher) => typeof matcher === "function";
var nameToOptions = (options) => typeof options === "string" ? { name: options } : options;
var Route = class _Route {
  ...
  validate() {
    ...
    if (!_Route.registeredMatchers.some(({ name }) => name in this)) {
      throw new Error(
  ...
  init(args) {
    ...
    if (nameOrOptions) {
      Object.assign(
        routeConfig,
        typeof nameOrOptions === "string" ? nameToOptions(nameOrOptions) : nameOrOptions
      );
    }
  ...
  generateMatcher() {
    const activeMatchers = _Route.registeredMatchers.map(
  ...
  delayResponse() {
    if (this.delay) {
      ...
      this.response = () => {
        return new Promise(
          (res) => setTimeout(() => res(response), this.delay)
        );
      };
    }
  }
  static addMatcher(matcher) {
    _Route.registeredMatchers.push(matcher);
  }
};
Route.registeredMatchers = [];
builtInMatchers.forEach(Route.addMatcher);
var FetchMock$1 = {};
var isName = (nameOrMatcher) => typeof nameOrMatcher === "string" && /^[\da-zA-Z-]+$/.test(nameOrMatcher);
var filterCallsWithMatcher = function(matcher, options = {}, calls) {
  ({ matcher } = new Route([{ matcher, response: "ok", ...options }], this));
  return calls.filter(
    ({ url, options: options2 }) => matcher(normalizeUrl(url), options2)
  );
};
var formatDebug = (func) => function(...args) {
  const result = func.call(this, ...args);
  return result;
};
var callObjToArray = (obj) => {
  if (!obj) {
    return void 0;
  }
...
var FetchMock = { ...FetchMock$2, ...FetchMock$3, ...FetchMock$1 };
...
FetchMock.createInstance = function() {
  ...
  instance.routes = instance._uncompiledRoutes.map(
    (config) => this.compileRoute(config)
  );
  instance.fallbackResponse = this.fallbackResponse || void 0;
